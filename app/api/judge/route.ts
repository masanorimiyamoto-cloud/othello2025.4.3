...
export async function POST(request: Request) {
  try {
    ...
    const form = await request.formData(); const file = form.get("photo"); const category = String(form.get("category") ?? "") as Category | "";
    ...
    const all = await listItems(); const candidates = (category ? all.filter((x) => x.category === category) : all).slice(0, MAX_COMPARE_ITEMS);
    if (!candidates.length) return NextResponse.json({ error: category ? "このカテゴリの登録品がありません" : "先に自分の物を登録してください" }, { status: 400 });
