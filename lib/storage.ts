...
async function readJson<T>(kind: keyof typeof indexes, fallback: T): Promise<T> {
  ...
  const response = await get(indexes[kind], { access: blobAccess() });
...
async function writeJson(kind: keyof typeof indexes, value: unknown) {
  ...
  if (!hasBlobConfig()) { await fs.mkdir(localRoot, { recursive: true }); await fs.writeFile(path.join(localRoot, `${kind}.json`), body); return; }
  await put(indexes[kind], body, { access: blobAccess(), allowOverwrite: true, contentType: "application/json" });
}
...
export async function removeItem(id: string) {
  ...
  if (hasBlobConfig()) await Promise.all(item.photos.map((p) => del(p.url).catch(() => undefined)));
...
export async function savePhoto(file: File, group: string): Promise<Photo> {
  ...
  if (hasBlobConfig()) { const blob = await put(`my-items/${pathname}`, file, { access: blobAccess(), contentType: file.type, addRandomSuffix: false }); return { id, url: blob.url, pathname: blob.pathname, createdAt }; }
...
export async function readPhoto(pathname: string) {
  if (hasBlobConfig()) return get(pathname, { access: blobAccess() });
